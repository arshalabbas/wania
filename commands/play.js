  ...
  async execute(message, args) {
    ...
    if (urlValid) {
      try {
        ...
        song = {
          ...
          image: songInfo.videoDetails.thumbnails[3].url,
          views: songInfo.videoDetails.viewCount,
          uploaded: songInfo.videoDetails.uploadDate
        };
      } catch (error) {
        console.error(error);
        return message.reply(error.message).catch(console.error);
    ...
    if (serverQueue) {
      const queueEmbed = new MessageEmbed()
        .setAuthor(`✅Song added to queue`, "https://i.ibb.co/W6hK79j/mimo-logo.png")
        .setTitle(song.title)
        .setURL(song.url)
        .setColor(16747679)
